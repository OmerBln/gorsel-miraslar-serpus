...
def train_one_epoch(model, dataloader, criterion, optimizer, scaler, device, epoch, num_epochs):
    """Tek bir epoch için eğitim döngüsü (AMP + Gradient Clipping)."""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for inputs, labels in progress_bar:
        ...
        # Gradient Clipping — büyük gradyanlardan koruma
...
def validate(model, dataloader, criterion, device, unknown_threshold):
    """Validasyon döngüsü — unknown threshold mekanizması ile."""
    model.eval()
    correct = 0
    total = 0
    unknown_count = 0
    ...
    with torch.no_grad():
        for inputs, labels in progress_bar:
            ...
            # Eşiği geçemeyenleri işaretle ve yanlış say
...
def build_model(num_classes, device):
    """ResNet-101 modelini Dropout katmanı ile oluşturur."""
    model = models.resnet101(weights=models.ResNet101_Weights.DEFAULT)

    # Son FC katmanını Dropout ile yeniden yapılandır
...
def cleanup_old_models(best_model_filename, model_dir='.'):
    """Eski best model dosyalarını silerek disk alanı tasarrufu sağlar."""
...
def main():
    # --- Data Augmentation (genişletilmiş) ---
    data_transforms = {
    ...
    # torch.compile ile ek hız kazanımı (PyTorch 2.x+)
    # Windows'ta Triton hatası verebileceği için devre dışı bırakıldı
    # if hasattr(torch, 'compile'):
    #     model = torch.compile(model)
    #     logger.info("torch.compile aktif edildi.")
    ...
    # Pretrained katmanlar düşük lr, yeni FC katmanı yüksek lr
    fc_params = list(model.parameters())[-2:]  # FC weight + bias (veya compile sonrası)
    ...
    # CosineAnnealingWarmRestarts — periyodik lr yeniden başlatma
    ...
    FREEZE_EPOCHS = 3  # İlk N epoch'ta sadece FC eğitilir
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # --- Her epoch için detaylı terminal çıktısı ---
